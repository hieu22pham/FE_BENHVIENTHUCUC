const actionTypes = Object.freeze({
    //app
    APP_START_UP_COMPLETE: "APP_START_UP_COMPLETE",
    SET_CONTENT_OF_CONFIRM_MODAL: "SET_CONTENT_OF_CONFIRM_MODAL",
    CHANGE_LANGUAGE: "CHANGE_LANGUAGE",
    DiSPLAY_LOADING: "DiSPLAY_LOADING",
    HIDE_LOADING: "HIDE_LOADING",

    //admin
    ADMIN_LOGIN_SUCCESS: "ADMIN_LOGIN_SUCCESS",
    ADMIN_LOGIN_FAIL: "ADMIN_LOGIN_FAIL",

    //user
    ADD_USER_SUCCESS: "ADD_USER_SUCCESS",

    USER_LOGIN_SUCCESS: "USER_LOGIN_SUCCESS",
    USER_LOGIN_SUCCESS2: "USER_LOGIN_SUCCESS2",
    USER_LOGIN_FAIL: "USER_LOGIN_FAIL",
    PROCESS_LOGOUT: "PROCESS_LOGOUT",

    //admin
    FETCH_GENDER_START: "FETCH_GENDER_START",
    FETCH_GENDER_SUCCESS: "ETCH_GENDER_SUCCESS",
    FETCH_GENDER_FAILED: "FETCH_GENDER_FAILED",

    FETCH_POSITION_START: "FETCH_POSITION_START",
    FETCH_POSITION_SUCCESS: "ETCH_POSITION_SUCCESS",
    FETCH_POSITION_FAILED: "FETCH_POSITION_FAILED",

    FETCH_ROLE_START: "FETCH_ROLE_START",
    FETCH_ROLE_SUCCESS: "ETCH_ROLE_SUCCESS",
    FETCH_ROLE_FAILED: "FETCH_ROLE_FAILED",

    CREATE_USER_SUCCESS: "CREATE_USER_SUCCESS",
    CREATE_USER_FAILED: "CREATE_USER_FAILED",

    EDIT_USER_SUCCESS: "EDIT_USER_SUCCESS",
    EDIT_USER_FAILED: "EDIT_USER_FAILED",

    DELETE_USER_SUCCESS: "DELETE_USER_SUCCESS",
    DELETE_USER_FAILED: "DELETE_USER_FAILED",

    GET_ALL_USER_SUCCESS: "GET_ALL_USER_SUCCESS",
    GET_ALL_USER_FAILED: "GET_ALL_USER_FAILED",

    GET_TOP_DOCTORS_SUCCESS: "GET_TOP_DOCTORS_SUCCESS",
    GET_TOP_DOCTORS_FAILED: "GET_TOP_DOCTORS_FAILED",

    GET_ALL_DOCTORS_SUCCESS: "GET_ALL_DOCTORS_SUCCESS",
    GET_ALL_DOCTORS_FAILED: "GET_ALL_DOCTORS_FAILED",

    SAVE_DETAIL_DOCTOR_SUCCESS: "SAVE_DETAIL_DOCTOR_SUCCESS",
    SAVE_DETAIL_DOCTOR_FAILED: "SAVE_DETAIL_DOCTOR_FAILED",

    GET_DETAIL_DOCTOR_SUCCESS: "GET_DETAIL_DOCTOR_SUCCESS",
    GET_DETAIL_DOCTOR_FAILED: "GET_DETAIL_DOCTOR_FAILED",

    GET_ALLCODE_SCHEDULE_TIME_SUCCESS: "GET_ALLCODE_SCHEDULE_TIME_SUCCESS",
    GET_ALLCODE_SCHEDULE_TIME_FAILED: "GET_ALLCODE_SCHEDULE_TIME_FAILED",

    FETCH_REQUIRED_DOCTOR_INFOR_START: "FETCH_REQUIRED_DOCTOR_INFOR_START",
    FETCH_REQUIRED_DOCTOR_INFOR_SUCCESS: "ETCH_REQUIRED_DOCTOR_INFOR_SUCCESS",
    FETCH_REQUIRED_DOCTOR_INFOR_FAILED: "FETCH_REQUIRED_DOCTOR_INFOR_FAILED",

    IS_SHOW_LOADING: "IS_SHOW_LOADING",

    CANCLE_BOOKING_SUCCESS: "CANCLE_BOOKING_SUCCESS",
    CANCLE_BOOKING_FAILED: "CANCLE_BOOKING_FAILED",

    CHANGE_PASSWORD_SUCCESS: "CHANGE_PASSWORD_SUCCESS",
    CHANGE_PASSWORD_FAILED: "CHANGE_PASSWORD_FAILED",

    GET_ALL_GENDER_SUCCESS: "GET_ALL_GENDER_SUCCESS",
    GET_ALL_GENDER_FAILED: "GET_ALL_GENDER_FAILED",

    GET_ALL_CLINIC_SUCCESS: "GET_ALL_CLINIC_SUCCESS",
    GET_ALL_CLINIC_FAILED: "GET_ALL_CLINIC_FAILED",
});

export default actionTypes;
